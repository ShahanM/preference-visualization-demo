...
export default function DiscreteDecoupled({ itemdata }) {
    ...
    const mylikes = useMemo(() => Object.entries(itemdata['movies']).filter((d) =>
        parseFloat(d[1].user_score) > likeCuttoff).map((d) => d[1]), [itemdata])

    const commlikes = useMemo(() => Object.entries(itemdata['movies']).filter((d) =>
        parseFloat(d[1].community_score) > likeCuttoff).map((d) => d[1]), [itemdata])

    const mydislikes = useMemo(() => Object.entries(itemdata['movies']).filter((d) =>
        parseFloat(d[1].user_score) <= dislikeCuttoff
    ).map((d) => d[1]), [itemdata])

    const commdislikes = useMemo(() => Object.entries(itemdata['movies']).filter((d) =>
        parseFloat(d[1].community_score) <= dislikeCuttoff
    ).map((d) => d[1]), [itemdata])
                    ...
                    <Col xl={8} lg={9} md={8} sm={12}>
                        <Row style={{ margin: "0 0 2em 0" }}>
                            <Row style={{ ...testContainerStyle, padding: "0" }}>
                                ...
                                <Col lg={6} style={{ ...testContainerStyle, textAlign: "center" }}>
                            ...
                            <Row style={{ ...testContainerStyle, padding: "0" }}>
                                ...
                                <Col lg={6} style={testContainerStyle}>
                                    {mylikes.length > 0 ?
                            ...
                            <Row style={{ testContainerStyle, padding: "0" }}>
                                ...
                                <Row style={{ ...testContainerStyle, padding: "0" }}>
                                    {/* <Col lg={2} style={testContainerStyle}></Col> */}
                                    <Col lg={6} style={{ ...testContainerStyle, textAlign: "center" }}>
                                        <p style={{ fontWeight: "bold" }}>Community Likes</p>
                                    </Col>
                                    <Col lg={6} style={{ testContainerStyle, textAlign: "center" }}>
                                        <p style={{ fontWeight: "bold" }}>Community Dislikes</p>
                                    </Col>
                                </Row>
                                <Row style={{ ...testContainerStyle, padding: "0" }}>
                                    <Col lg={6} style={testContainerStyle}>
